...
public class MybatisGeneratorBridge {
    ...
    public void generate() throws Exception {
        ...
        // 按照字段自增减方法
        PluginConfiguration columnPluginConfiguration = new PluginConfiguration();
        columnPluginConfiguration.addProperty("type", "com.zzg.mybatis.generator.plugins.ColumnSelfPlusMinusPlugin");
        columnPluginConfiguration.setConfigurationType("com.zzg.mybatis.generator.plugins.ColumnSelfPlusMinusPlugin");
        context.addPluginConfiguration(columnPluginConfiguration);

        PluginConfiguration columnPlugin = new PluginConfiguration();
        columnPlugin.addProperty("type", "com.zzg.mybatis.generator.plugins.ColumnSelfPlusMinusByMapPlugin");
        columnPlugin.setConfigurationType("com.zzg.mybatis.generator.plugins.ColumnSelfPlusMinusByMapPlugin");
        context.addPluginConfiguration(columnPlugin);
...
//生成cache
        if(generatorConfig.isCache()) {
            PluginConfiguration myCachePlugin = new PluginConfiguration();
            myCachePlugin.addProperty("type", "org.mybatis.generator.plugins.CachePlugin");
            myCachePlugin.setConfigurationType("org.mybatis.generator.plugins.CachePlugin");
            if(StringUtils.isNotBlank(generatorConfig.getCacheType())) {
                myCachePlugin.addProperty(CachePlugin.CacheProperty.TYPE.getPropertyName(), generatorConfig.getCacheType());
            }
            if(StringUtils.isNotBlank(generatorConfig.getFlashInterval())) {
                myCachePlugin.addProperty(CachePlugin.CacheProperty.FLUSH_INTERVAL.getPropertyName(), generatorConfig.getFlashInterval());
            }
            context.addPluginConfiguration(myCachePlugin);
        }
